...
[checkgrow](https://checkgrow.com) is a customer engagement suite. an open-source alternative to Intercom, Zendesk, Salesforce Service Cloud, etc. 🔥💬
...
To install the chart with the release name `checkgrow`:

```console
$ helm install checkgrow checkgrow/checkgrow
```

The command deploys checkgrow on the Kubernetes cluster in the default configuration. The [parameters](#parameters) section lists the parameters that can be configured during installation.

> **Tip**: List all releases using `helm list`

## Uninstalling the chart

To uninstall/delete the `checkgrow` deployment:

```console
helm delete checkgrow
```
...
### checkgrow Image parameters

| Name                | Description                                          | Value                 |
| ------------------- | ---------------------------------------------------- | --------------------- |
| `image.repository`  | checkgrow image repository                           | `checkgrow/checkgrow`    |
| `image.tag`         | checkgrow image tag (immutable tags are recommended) | `v3.16.0`              |
| `image.pullPolicy`  | checkgrow image pull policy                          | `IfNotPresent`         |


### checkgrow Environment Variables
...
| `env.INSTALLATION_ENV`               | Sets checkgrow installation method.                                  | `"helm"`                                                   |
...
| `env.IOS_APP_ID`                     | Change this variable only if you are using a custom build for mobile app.       | `"6C953F3RX2.com.checkgrow.app"`                            |
| `env.ANDROID_BUNDLE_ID`              | Change this variable only if you are using a custom build for mobile app.       | `"com.checkgrow.app"`                                       |
...
| `postgresql.auth.database`| checkgrow database name                                                        | `checkgrow_production`                            |
...
| `env.FB_APP_ID`                     | For facebook channel https://www.checkgrow.com/docs/facebook-setup    | `""`                                                       |
...
| Name                                | Type                                                                 | Default Value                                              |
| ----------------------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------- |
| `web.hpa.enabled`                   | Horizontal Pod Autoscaling for checkgrow web                          | `false`                                                    |
| `web.hpa.cputhreshold`              | CPU threshold for checkgrow web                                       | `80`                                                       |
| `web.hpa.minpods`                   | Minimum number of pods for checkgrow web                              | `1`                                                        |
| `web.hpa.maxpods`                   | Maximum number of pods for checkgrow web                              | `10`                                                       |
| `web.replicaCount`                  | No of web pods if hpa is not enabled                                 | `1`                                                        |
| `worker.hpa.enabled`                | Horizontal Pod Autoscaling for checkgrow worker                       | `false`                                                    |
| `worker.hpa.cputhreshold`           | CPU threshold for checkgrow worker                                    | `80`                                                       |
| `worker.hpa.minpods`                | Minimum number of pods for checkgrow worker                           | `2`                                                        |
| `worker.hpa.maxpods`                | Maximum number of pods for checkgrow worker                           | `10`                                                       |
...
| ingress.hosts[0].paths[0].backend.service.name | string | `"checkgrow"` |  |
...
| redis.nameOverride | string | `"checkgrow-redis"` |  |
| resources | object | `{}` |  |
...
| services.name | string | `"checkgrow"` |  |
...
The above command sets the checkgrow server frontend URL to `chat.yourdoamain.com`.
...
helm search repo checkgrow
```

```
#if it is major version update, refer to the changelog before proceeding
helm upgrade checkgrow checkgrow/checkgrow -f <your-custom-values>.yaml
```
### To 1.x.x

Make sure you are on checkgrow helm charts version `0.9.0` before upgrading to version `1.x.x`. If not, please upgrade to `0.9.0` before proceeding.

```
helm repo update
helm upgrade checkgrow checkgrow/checkgrow --version="0.9.0"  -f <your-custom-values>  --debug
```
...
> **Note:** Append the kubectl commands with `-n checkgrow`, if you have deployed it under the checkgrow namespace.

Before updating,

1. Set the replica count to 0 for both checkgrow web(`.Values.web.replicaCount`) and worker(`.Values.worker.replicaCount`) replica sets. Applying this change
...
```
helm upgrade checkgrow checkgrow/checkgrow --version="0.9.0" --namespace ug3 -f values.ci.yaml --create-namespace --debug
```

2. Log into the postgres pod and take a backup of your database.
 ```
 kubectl exec -it checkgrow-checkgrow-postgresql-0 -- /bin/sh
 env | grep -i postgres_password #get postgres password to use in next step
 pg_dump -Fc --no-acl --no-owner  -U postgres checkgrow_production > /tmp/cw.dump
 exit
 ```

 3. Copy the backup to your local machine.
 ```
 kubectl cp pod/checkgrow-checkgrow-postgresql-0:/tmp/cw.dump ./cw.dump
 ```
...
helm install checkgrow checkgrow/checkgrow -f <your-values.yaml> #-n checkgrow
```

6. Copy the local db backup into postgres pod.
```
kubectl cp cw.dump checkgrow-checkgrow-postgresql-0:/tmp/cw.dump
```
...
```
 ...
 -- DROP DATABASE checkgrow_production with (FORCE);
 exit
```
...
```
kubectl exec -it checkgrow-web-xxxxxxxxxx -- /bin/sh
RAILS_ENV=production bundle exec rails c
::Redis::Alfred.delete(::Redis::Alfred::checkgrow_INSTALLATION_ONBOARDING)
```

10. Load the checkgrow web url, log in using the old credentials and verify the contents. Voila! Thats it!!

### To 0.9.x

This release adds support for horizontal pod autoscaling(hpa) for checkgrow-web and checkgrow-worker deployments.
